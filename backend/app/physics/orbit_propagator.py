"""Keplerian Orbit Propagator with J2 Perturbations & Eclipse Geometry."""
...
def get_sun_position_eci(sim_time_s: float) -> np.ndarray:
    ...
    # Earth orbital period around sun: ~365.25 days
...
def is_satellite_sunlit(r_eci: np.ndarray, sim_time_s: float) -> bool:
    ...
    # Projection along Sun vector
    proj = np.dot(r_eci, sun_unit)
    if proj > 0:
        # Facing the sun -> illuminated
        return True
...
def propagate_orbit(
    ...
    # Convert ECEF to Geodetic Latitude, Longitude, Altitude (WGS-84 Spherical approx / Bowring)
...
def create_initial_constellation(
    ...
    """Generates a Walker-Delta type LEO constellation."""
    ...
    for p in range(num_planes):
        ...
        for s in range(sats_per_plane):
            # Stagger mean anomaly per plane
            ...
            keplerian = KeplerianElements(
                ...
                eccentricity=0.001,  # Near-circular LEO
